...
export class DesktopChatAPI implements IChatAPI {
  ...
  async sendMessage(
    ...
    try {
      const stream = chatService.sendMessage(chatId, content, coreAttachments);
  ...
  async seedDemoChats(): Promise<void> {
    ...
    const stream1 = chatService.sendMessage(
      chat1Id,
    ...
    const stream3 = chatService.sendMessage(
      chat3Id,
      "Show me some math notation"
    );
